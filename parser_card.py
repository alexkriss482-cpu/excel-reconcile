"""Парсер реестра оплат платежной картой."""
...
class CardParseError(ValueError):
    """Ошибка разбора реестра оплат картой."""


def _require_column(df: pd.DataFrame, key: str) -> str:
    column = find_column(df.columns, COLUMN_MAPPINGS["card"][key])
    if column is None:
        aliases = ", ".join(COLUMN_MAPPINGS["card"][key])
        raise CardParseError(
            f"В реестре оплат картой не найдена колонка «{key}». "
...
def _load_registry(filepath: str) -> tuple[pd.DataFrame, int]:
    ...
    if header_row is None:
        raise CardParseError("Не найдена строка заголовков с колонками «Дата» и «Сумма».")

    df = pd.read_excel(filepath, engine="openpyxl", header=header_row)
    return df, len(df)


def parse_card(filepath: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    Загрузить и обработать реестр оплат платежной картой.
    ...
    doc_col = find_column(df.columns, COLUMN_MAPPINGS["card"]["document_type"])

    work = df.copy()
    if doc_col:
        work = work[work[doc_col].astype(str).str.strip().isin(CARD_ALLOWED_DOCUMENT_TYPES)]
